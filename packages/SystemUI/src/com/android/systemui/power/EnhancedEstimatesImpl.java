...
public final class EnhancedEstimatesImpl implements EnhancedEstimates {
    ...
    private final Estimate EMPTY_ESTIMATE =
            new Estimate(
                    EstimateKt.ESTIMATE_MILLIS_UNKNOWN,
                    false,
                    EstimateKt.AVERAGE_TIME_TO_DISCHARGE_UNKNOWN);

    private final Duration DAY = Duration.ofDays(1L);
    private final long HOUR = Duration.ofHours(1L).toMillis();
    private final long THREE_HOURS = Duration.ofHours(3L).toMillis();
    private final long FIFTEEN_MINUTES = Duration.ofMinutes(15L).toMillis();
    ...
    public Estimate getEstimate() {
        final Uri build =
                new Uri.Builder()
                        ...
                        .build();
        try (final Cursor query =
                mContext.getContentResolver().query(build, null, null, null, null)) {
    ...
    private Estimate parseEstimateFromCursor(Cursor query) {
        ...
        if (batteryLifecolumnIndex != -1) {
            ...
            if (averageBatteryLife != -1L) {
                final long duration =
                        Duration.ofMillis(averageBatteryLife).compareTo(DAY) >= 0
                                ? HOUR
                                : FIFTEEN_MINUTES;
