...
public class BcSmartspaceView extends FrameLayout
    ...
    public final void onPageScrollStateChanged(int state) {
      ...
      if (state == ViewPager2.SCROLL_STATE_IDLE) {
        if (mConfigProvider.isSwipeEventLoggingEnabled()
            ...
            && mSwipedCardPosition != mViewPager2.getCurrentItem()
            && mAdapter.getCardAtPosition(mSwipedCardPosition) != null) {
          BcSmartspaceCardLogger.log(
              BcSmartspaceEvent.SMARTSPACE_CARD_SWIPE,
              mAdapter.getCardAtPosition(mSwipedCardPosition).getLoggingInfo());
        }
        mSwipedCardPosition = null;
      }
    }
    ...
    public final void onPageScrollStateChanged(int state) {
      ...
      if (state == ViewPager.SCROLL_STATE_IDLE) {
        if (mConfigProvider.isSwipeEventLoggingEnabled()
            ...
            && (cardAtPosition = mAdapter.getCardAtPosition(mSwipedCardPosition.intValue()))
                != null) {
          BcSmartspaceCardLogger.log(
              BcSmartspaceEvent.SMARTSPACE_CARD_SWIPE, cardAtPosition.getLoggingInfo());
        }
        mSwipedCardPosition = null;
  ...
  public static void onViewPagerPageSelected(BcSmartspaceView view, int position) {
    ...
    if (currentTarget == null) {
      Log.w(
          "BcSmartspaceView",
          "Cannot notify target shown smartspace event: shown card smartspace target" + " null.");
      return;
    }
  ...
  public int getCurrentCardTopPadding() {
    ...
    BcSmartspaceRemoteViewsCard remoteViewsCard = mAdapter.getRemoteViewsCardAtPosition(position);
    ...
    if (event == BcSmartspaceEvent.SMARTSPACE_CARD_RECEIVED) {
      try {
        receivedLatencyMillis =
            (int) Instant.now().minusMillis(target.getCreationTimeMillis()).toEpochMilli();
    ...
    if (mViewPager != null) {
      if (mAdapter instanceof CardPagerAdapter) {
        ...
        try {
          ...
          Log.w(
              "BcSmartspaceView", "Unable to register Smartspace Background Settings observer.", e);
        }
        onBackgroundToggled();
  ...
  public final void onDetachedFromWindow() {
    ...
    mBgHandler.post(
        () -> {
          ...
          getContext().getContentResolver().unregisterContentObserver(mBackgroundToggleObserver);
        });
    if (mViewPager != null) {
  ...
  public final void onVisibilityAggregated(boolean isVisible) {
    ...
    if (mDataProvider != null) {
      mDataProvider
          .getEventNotifier()
          .notifySmartspaceEvent(new SmartspaceTargetEvent.Builder(isVisible ? 6 : 7).build());
    }
