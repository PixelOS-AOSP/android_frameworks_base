...
public final class CardRecyclerViewAdapter
    ...
    if (targets.isEmpty()) {
      targets.add(
          new SmartspaceTarget.Builder(
                  ...
                  new ComponentName(root.getContext(), CardRecyclerViewAdapter.class),
  ...
  public BcSmartspaceRemoteViewsCard getRemoteViewsCardAtPosition(int position) {
    SmartspaceCard card = getCardAtPosition(position);
    return card instanceof BcSmartspaceRemoteViewsCard ? (BcSmartspaceRemoteViewsCard) card : null;
  }
  ...
  public final ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
      ...
      if (secondaryCardResId != null) {
        BcSmartspaceCardSecondary bcSmartspaceCardSecondary =
            (BcSmartspaceCardSecondary)
                inflater.inflate(secondaryCardResId, (ViewGroup) templateCard, false);
      ...
      if (legacySecondaryCardResourceIdSet.contains(viewType)
        ...
        if (secondaryCardResId != null) {
          legacyCard.setSecondaryCard(
              (BcSmartspaceCardSecondary)
                  inflater.inflate(secondaryCardResId, (ViewGroup) legacyCard, false));
        }
        card = legacyCard;
      } else {
    ...
    if (card instanceof BcSmartspaceRemoteViewsCard) {
      ...
      marginLayoutParams.topMargin =
          parent.getContext().getResources().getDimensionPixelSize(R.dimen.background_top_padding);
  ...
  public final void refreshCardPaddings() {
    ...
    for (int i = 0; i < viewHolders.size(); i++) {
      ...
      BaseTemplateCard templateCardAtPosition = getTemplateCardAtPosition(viewHolders.keyAt(i));
  ...
  public final void setDozeAmount(float dozeAmount) {
    ...
    } else if (currentBackgroundDrawable != backgroundDrawable && needToSetToLockscreenTargets()) {
  ...
  */
  public final void updateTargetVisibility(Runnable runnable, boolean z) {
    ...
    if (smartspaceTargets != targets) {
      if (dozeAmount == 1.0f || (dozeAmount >= 0.36f && transitioningTo == TransitionType.TO_AOD)) {
        z2 = true;
        boolean z3 = smartspaceTargets == lockscreenTargets && needToSetToLockscreenTargets();
        if (!z2) {
          Log.d("SsCardRecyclerViewAdapter", "Updating Smartspace targets to targets for AOD");
          smartspaceTargets = targets;
        } else if (z3) {
          Log.d(
              "SsCardRecyclerViewAdapter", "Updating Smartspace targets to targets for Lockscreen");
