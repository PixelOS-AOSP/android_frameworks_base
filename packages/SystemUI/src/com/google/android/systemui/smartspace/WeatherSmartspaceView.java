...
public class WeatherSmartspaceView extends LinearLayout
    ...
    if (TextUtils.equals(mUiSurface, BcSmartspaceDataPlugin.UI_SURFACE_LOCK_SCREEN_AOD)) {
      ...
      mIsAodEnabled =
          Settings.Secure.getIntForUser(
                  getContext().getContentResolver(), "doze_always_on", 0, getContext().getUserId())
              == 1;
    }
      ...
      if (!hasValidTemplate) {
        ...
        if (subItemInfo == null) {
          ...
          ContentDescriptionUtil.setFormattedContentDescription(
              "WeatherSmartspaceView",
              mView,
              SmartspaceUtils.isEmpty(subItemInfo.getText()) ? "" : subItemInfo.getText().getText(),
              subItemInfo.getIcon() != null ? subItemInfo.getIcon().getContentDescription() : "");
  ...
  public WeatherSmartspaceView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray obtainStyledAttributes =
        context.getTheme().obtainStyledAttributes(attrs, R.styleable.WeatherSmartspaceView, 0, 0);
    try {
      int iconSize =
      ...
      int iconInset =
          obtainStyledAttributes.getDimensionPixelSize(
              0,
              context.getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_inset));
